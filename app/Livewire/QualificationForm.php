...
class QualificationForm extends Component
{
    ...
    public function submit()
    {
        ...
        if (empty($this->specificRequests) && empty($this->otherSpecificRequests) && empty($this->generalRequests) && empty($this->otherGeneralRequests) && empty(trim($this->otherRequest))) {
    ...
    {
        $current = array_values($this->countries);

        if ($country !== 'France' && count($current) === 1 && $current[0] === 'France' && !$this->countriesTouched) {
    ...
    public function addOtherCountry($country)
    {
        ...
        if (in_array($country, self::PREDEFINED_COUNTRIES, true) || $this->geographyService->isValidCountry($country)) {
            $current = array_values($this->countries);
            if (!$this->countriesTouched && count($current) === 1 && $current[0] === 'France') {
                $this->countries = [$country];
            } else {
                $this->countries[] = $country;
            }
