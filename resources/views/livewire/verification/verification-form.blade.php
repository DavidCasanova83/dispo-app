<div class="flex h-full w-full flex-1 flex-col gap-6 max-w-2xl mx-auto"
    ...
    {{-- Bandeau message admin (priorité haute si re-vérification demandée) --}}
    @if ($adminMessage)
    ...
    <div class="rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 px-4 py-3 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <p class="text-sm text-gray-700 dark:text-gray-300">
            Avant de remplir le formulaire, ouvrez la page et lisez son contenu.
        </p>
        <a href="{{ $page->urlForLanguage($language) }}" target="_blank" rel="noopener noreferrer"
            class="inline-flex items-center justify-center px-4 py-2 text-sm font-medium text-white bg-gray-900 hover:bg-gray-800 dark:bg-white dark:text-gray-900 dark:hover:bg-gray-100 rounded">
    ...
    <form wire:submit.prevent="submit" class="space-y-10">
        ...
        <section class="space-y-6">
            <header class="border-b border-gray-200 dark:border-gray-700 pb-2">
                <h2 class="text-lg font-semibold text-gray-900 dark:text-white">Vérification factuelle</h2>
                <p class="text-xs text-gray-500 dark:text-gray-400 mt-0.5">Section obligatoire</p>
            </header>

            {{-- Q1 --}}
            <div>
                <p class="text-base font-medium text-gray-900 dark:text-white mb-3">
                    1. Le contenu de la page est-il exact et à jour ?
                </p>
                <div class="space-y-2">
                    <label class="flex items-start gap-3 p-3 rounded border border-gray-200 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700/30 cursor-pointer">
                        <input type="radio" wire:model.live="contentOk" :value="true" value="1"
                    ...
                    <label class="flex items-start gap-3 p-3 rounded border border-gray-200 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700/30 cursor-pointer">
                        <input type="radio" wire:model.live="contentOk" :value="false" value="0"
                            class="mt-0.5 text-gray-900 focus:ring-gray-900 dark:text-white dark:focus:ring-white">
                ...
                @if ($contentOk === false)
                    <div class="mt-3">
                        ...
                        <textarea wire:model.live.debounce.500ms="contentComment"
                            x-model="contentComment"
            ...
            <div>
                <p class="text-base font-medium text-gray-900 dark:text-white mb-3">
                    2. Les images et les liens fonctionnent-ils ?
                </p>
                <div class="space-y-2">
                    <label class="flex items-start gap-3 p-3 rounded border border-gray-200 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700/30 cursor-pointer">
                        <input type="radio" wire:model.live="mediaOk" :value="true" value="1"
                            class="mt-0.5 text-gray-900 focus:ring-gray-900 dark:text-white dark:focus:ring-white">
                        <span class="text-sm text-gray-900 dark:text-white">Oui, tout fonctionne et les images sont pertinentes</span>
                    </label>
                    <label class="flex items-start gap-3 p-3 rounded border border-gray-200 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700/30 cursor-pointer">
                        <input type="radio" wire:model.live="mediaOk" :value="false" value="0"
                            class="mt-0.5 text-gray-900 focus:ring-gray-900 dark:text-white dark:focus:ring-white">
                ...
                @if ($mediaOk === false)
                    <div class="mt-3">
                        ...
                        <textarea wire:model.live.debounce.500ms="mediaComment"
                            x-model="mediaComment"
                            rows="6" maxlength="5000"
            ...
            <div>
                <p class="text-base font-medium text-gray-900 dark:text-white mb-2">
                    3. Remarques ou suggestions
                    <span class="text-gray-500 text-sm font-normal">— optionnel</span>
                </p>
                <textarea wire:model.live.debounce.500ms="remarks"
                    x-model="remarks"
        ...
        <section class="space-y-6">
            <header class="border-b border-gray-200 dark:border-gray-700 pb-2">
                <h2 class="text-lg font-semibold text-gray-900 dark:text-white">Pour aller plus loin</h2>
                <p class="text-xs text-gray-500 dark:text-gray-400 mt-0.5">
                    Section optionnelle. Vous pouvez envoyer la vérification dès que la première section est remplie.
                </p>
            </header>
            ...
            <div>
                <p class="text-base font-medium text-gray-900 dark:text-white mb-3">
                    4. Le contenu de cette page vous semble-t-il pertinent ?
                </p>
                <div class="space-y-2">
            ...
            <div>
                <p class="text-base font-medium text-gray-900 dark:text-white mb-3">
                    5. Faudrait-il ajouter du contenu sur cette page ?
                </p>
                <div class="space-y-2">
                ...
                @php
                    $hasYesAdd = ! empty(array_filter($contentToAdd, fn ($c) => $c !== 'none'));
                @endphp
                @if ($hasYesAdd)
                    <div class="mt-3">
                        <label class="block text-sm font-medium text-gray-900 dark:text-white mb-1.5">
                        ...
                        <textarea wire:model.live.debounce.500ms="contentToAddDetails"
                            x-model="contentToAddDetails"
            ...
            <div>
                <p class="text-base font-medium text-gray-900 dark:text-white mb-3">
                    6. Y a-t-il du contenu à supprimer ou à alléger ?
                </p>
                <div class="space-y-2">
                ...
                @if ($contentToRemove === 'yes')
                    <div class="mt-3">
                        ...
                        <textarea wire:model.live.debounce.500ms="contentToRemoveDetails"
                            x-model="contentToRemoveDetails"
                            rows="5" maxlength="5000"
            ...
            <div>
                <p class="text-base font-medium text-gray-900 dark:text-white mb-3">
                    7. Si vous étiez un visiteur, cette page vous suffirait-elle ?
                </p>
                <div class="space-y-2">
                ...
                @if (in_array($visitorPerspective, ['partial', 'no'], true))
                    <div class="mt-3">
                        ...
                        <textarea wire:model.live.debounce.500ms="visitorUnanswered"
                            x-model="visitorUnanswered"
                            rows="5" maxlength="5000"
                            placeholder="Ex : « Comment y accéder en transports en commun ? » « Quels sont les tarifs pour les enfants ? »"
                            class="w-full rounded border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 px-3 py-2 text-sm text-gray-900 dark:text-white"></textarea>
            ...
            <div>
                <p class="text-base font-medium text-gray-900 dark:text-white mb-2">
                    9. Idées d'amélioration pour cette page
                    <span class="text-gray-500 text-sm font-normal">— optionnel</span>
                </p>
                <textarea wire:model.live.debounce.500ms="suggestions"
                    x-model="suggestions"
                    rows="6" maxlength="5000"
        ...
        {{-- Actions --}}
        <div class="flex flex-col sm:flex-row sm:justify-end gap-2 pt-4 border-t border-gray-200 dark:border-gray-700">
            <a href="{{ route('verification.index') }}" wire:navigate
    ...
    return Object.assign(data, {
        init() {
            ...
            const persist = () => {
                try {
                    const out = {};
                    fields.forEach(f => { out[f] = this[f]; });
                    localStorage.setItem(key, JSON.stringify(out));
                } catch (e) { /* ignore */ }
            };

            fields.forEach(f => this.$watch(f, persist));
        },
        clearDraft() {
