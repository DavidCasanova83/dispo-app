        ...
        <div class="grid grid-cols-2 sm:grid-cols-4 gap-2 mt-2">
            @foreach (['Patou', 'Pluies', 'Enfant en bas age', 'Réclamation'] as $option)
                <button type="button" wire:click="toggleOtherGeneralRequest({{ json_encode($option) }})"
                    @class([
                        'px-3 py-1.5 rounded-lg font-medium transition-all duration-200 border text-center text-sm',
                        'bg-[#3E9B90] text-white shadow-md border-[#3E9B90]' => in_array($option, $otherGeneralRequests),
                        'bg-white text-gray-700 border-gray-300 hover:border-[#3E9B90] hover:text-[#3E9B90] dark:bg-gray-800 dark:text-gray-200 dark:border-gray-600 dark:hover:border-[#3E9B90]' => !in_array($option, $otherGeneralRequests),
                    ])>
                    {{ $option }}
                </button>
            @endforeach
