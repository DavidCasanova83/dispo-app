<div x-show="$wire.currentStep === 2" x-transition:enter="transition ease-out duration-300"
    ...
    <h2 class="text-2xl font-bold mb-8 text-gray-900 dark:text-white">Étape 2 : Profil</h2>
        ...
        <div class="grid grid-cols-3 gap-2 mb-3">
            @foreach (['Seul', 'Couple', 'Famille', 'Groupe d\'amis', 'Groupe Scolaire', 'Groupe de voyages', 'Groupe famille'] as $profileOption)
                <button type="button" wire:click="$set('profile', {{ json_encode($profileOption) }})"
                    ...
                    class="px-4 py-2.5 rounded-lg font-medium transition-all duration-200 border text-center text-sm disabled:opacity-50 disabled:cursor-not-allowed">
                    {{ $profileOption }}
                </button>
            @endforeach
        ...
        <div class="grid grid-cols-3 gap-2 mb-3">
            @foreach (['0-6', '6-12', '12-18', '18-25', '25-40', '40-60', '60+'] as $ageOption)
                <button type="button" wire:click="toggleAgeGroup('{{ $ageOption }}')" :disabled="$wire.ageUnknown"
                    @class([
                        'px-4 py-2.5 rounded-lg font-medium transition-all duration-200 border text-center text-sm disabled:opacity-50 disabled:cursor-not-allowed',
